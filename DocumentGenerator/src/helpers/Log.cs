...
public class Log
{
            ...
            if (log == string.Empty || log.ToLower() != "false")
            {
                ...
                if (System.IO.File.Exists(logName))
                {
                    ...
                    if ((logText.Length > MAXSIZE))
                        System.IO.File.WriteAllText(logName, logText.Substring(MAXSIZE / 2) + string.Format("{0}----------------------------------------------------------------------------------------------------------------------------------------------------------------{0}", Constants.vbCrLf));
                }

                // REM System.IO.File.AppendAllText(logName, String.Format("{0}----------------------------------------------------------------------------------------------------------------------------------------------------------------{0}", vbCrLf))
                Serilog.Log.Logger = new LoggerConfiguration().WriteTo.File(logName, Serilog.Events.LogEventLevel.Verbose, null/* Conversion error: Set to default value for this argument */, null/* Conversion error: Set to default value for this argument */, MAXSIZE * 4).CreateLogger();
    ...
    protected static string getSetting(string section, string name)
    {
    ...
    // Dim frame As System.Diagnostics.StackFrame = (New System.Diagnostics.StackTrace(True)).GetFrame(2)
    // Dim method As System.Reflection.MethodBase = frame.GetMethod()

    // Return String.Format("{0}.[{1}].({2})", method.DeclaringType.FullName, method.ToString(), frame.GetFileLineNumber)
    // End Function
